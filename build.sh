...
# Validate VARIANT
if [ "$VARIANT" != "npm" ] && [ "$VARIANT" != "code-writer-ts" ] && [ "$VARIANT" != "code-writer-py" ]; then
    echo "Error: VARIANT must be 'npm', 'code-writer-ts', or 'code-writer-py'" >&2
    exit 1
fi
...
# Read versions from YAML (shared across all variants)
...
if [ "$VARIANT" == "npm" ]; then
    ...
    TAG="agents-with-multica-code-writer-py:${PROJECT_VERSION}-${ARCH}"
else
    DOCKERFILE="${SCRIPT_DIR}/code-writer/ts/Dockerfile"
    TAG="agents-with-multica-code-writer-ts:${PROJECT_VERSION}-${ARCH}"
fi
...
if [ "$VARIANT" == "npm" ]; then
    ...
    UV_VERSION=$(python3 -c "import yaml; print(yaml.safe_load(open('${SCRIPT_DIR}/versions.yaml'))['code_writer']['uv_version'])" 2>&1) || {
